...
- Signal line = BELL ONLY: window.append_wake_signal writes `<wake.wake_signal_marker '[CORTEX-WAKE]'> HH:MM`; marrow UserPromptSubmit hook detects marker → injects full note (no note body/read errand in the line). type_wake_signal adds wake.rearm_suffix.
- P0 spawn-hint timing (wake.py _spawn_wake): record the NEW session transcript only after _wait_new_transcript (bounded poll ~8s for a jsonl newer than pre-spawn newest or mtime ≥ spawn_ts); timeout → record None, never the stale pre-spawn path (which drove an endless respawn loop every tick). None hint + alive window + no flag → plan=ear.
...
- launch_command(cfg, initial_prompt=None, resume_sid=None): `cd <cortex_home> && MARROW_CORTEX=1 MARROW_CHANNEL=ct claude --model <wake.window_model, default opus> [--effort] [--resume <sid>] [--dangerously-skip-permissions, default true] [initial_prompt]`. initial_prompt = emoji-only wake prompt (window.wake_prompt, default ☀️, zero readable text); marrow hook injects full note on it. resume_sid non-empty → `--resume <sid>` relaunch same conversation. Arm-ear mechanism retired.
- claude_session_id(cfg) = recorded transcript jsonl stem = conversation UUID for --resume (NOT the iTerm session_id). None when no transcript hint.
- _wait_ready polls session text for wake.ready_marker ('accept edits') up to 30s.
- respawn(cfg, initial_prompt=None, resume_sid=None): SIGTERM old claude, close session, _spawn(initial_prompt, resume_sid), persist sid. Fresh (resume_sid None) or resume (--resume) per caller. Silent — no notification on spawn (spawn_greeting/_notify removed; say() front+sound is the sole attention-getter).
...
- lie_down (lie_down.py): record tokens+net into ct_wake_log (sole writer of those columns; bare `except: pass` — known silent-drop finding) → clear due self_schedule entries → integration.lie_down floor redraw from now (explicit next_wake_min clamped, or dice; returns next_floor dt) → publish net via store_window_tokens → kill watchdog (SIGTERM, skip if self) → optional set_rotated → clear_awake. Result dict adds next_wake=HH:MM (local tz, marrow MCP wrapper surfaces it); CLI main prints result JSON.
...
- Known gaps: install.py (untested entirely), geofence same-minute-same-text dup.
