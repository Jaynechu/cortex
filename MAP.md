...
- ct-pause = pure stop: `lie_down(book_alarm=False)` books NO next wake (ledger cleared). ct-wake fires a round immediately; `ctl resume --shell cli` books a due-now alarm instead (reconcile picks it up) so the empty ledger cannot sleep forever. fuse/stale still book the default.
...
- Sections: header = 📍 location (if any) + merged leopard/Mac line, then Pending self-schedule (note.pending_window_min 15). No `Now HH:MM Day` line — per-turn hook already injects current time.
- Visible note = tag, 📍 (if present), `🐆 Cortex last wake: Xm ago [+ pause tag] | 💻 Mac is Active: <app>` (no computer status → 🐆 half alone, no pipe). Mac states: Active: <app>, Inactive: Xm <app>, Locked: Xh. turn_end_text + title default "" (omitted). "Wake:" reason line retired.
