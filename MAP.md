...
- **Config: marrow only.** `[cortex.breaker]` in `~/.config/marrow/config.toml` (defaults in marrow/config.default.toml): `enabled` / `fuse_threshold` / `window_hours` / `trip_message` / `pause_message` / `clear_message`. `breaker.settings()` reads that toml directly and layers it over `breaker.DEFAULTS`; missing file/section/key → defaults. Deliberately NOT duplicated into cortex.toml or the tg bridge config.
...
- `note_render --shell <non-cli>` diffs from that shell's ledger `last_note_row_id` (note.py:_shell_ledger_row_id, plain json read; missing file/key = full window). Every render (cli/unqualified included) prints the rendered subset's max row id on stderr as `cutoff_row_id=N` (nothing rendered = no line). Stays read-only: the feeder writes the cursor after a successful feed — tg ledger via synapse, cli wake_state via marrow `advance_cli_replay_cursor`.
