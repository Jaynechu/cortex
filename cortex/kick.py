...
A cli session pokes cortex awake to peek at a channel — the first message of the
day. This module does what the marrow user-wake reset does for cortex windows,
but for the sleeping case: under the wake_state flock + cancellation epoch it
...
def _reason_text(cfg: dict, kind: str) -> str:
    """The config kick-reason line for `kind`. Missing template -> ""."""
    kcfg = cfg.get("kick", {}) or {}
    return str(kcfg.get(f"reason_{kind}") or "").strip()
...
def kick(cfg: dict, kind: str) -> dict:
    """Run one kick. `kind` selects a config reason template. Returns a small
    result dict.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Poke cortex awake to peek at a channel (morning).")
    parser.add_argument("--kind", required=True, choices=("morning",),
                        help="reason template to render into the wakeup note")
    args = parser.parse_args(argv)
    cfg = config.load()
    result = kick(cfg, args.kind)
