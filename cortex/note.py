...
def gather(
    ...
    *,
    ...
    `settle` (default False): kick reasons are pending-visible — a read does NOT
    consume them; ONLY a real payload injection settles them (clear). settle=True
    is passed by injection paths (free-round ear write). The wake-assemble path
    leaves settle=False (its frozen note may be discarded — the marrow hook is the
    real injector). A passive re-render (consume_kick=False) PEEKS read-only so an
